...
import {
    ...
    Plus, ChevronRight, Search, Filter,
} from 'lucide-react'
...
export default function HospitalDashboard() {
    ...
    const stats = {
        total: claims.length,
        pending: claims.filter((c) => ['DRAFT', 'PENDING_DECISION'].includes(c.status)).length,
        review: claims.filter((c) => ['SUBMITTED_TO_INSURER', 'UNDER_REVIEW'].includes(c.status)).length,
        settled: claims.filter((c) => ['APPROVED', 'CASH_PAID'].includes(c.status)).length,
    }
    ...
    return (
        <>
            <PageHeader
                ...
                subtitle="Manage all patient claims from one place."
                action={
                    <Link
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <StatCard label="Total filed" value={stats.total} icon={<FileText className="w-4 h-4" />} />
                    <StatCard label="Pending decision" value={stats.pending} icon={<Clock className="w-4 h-4" />} color="amber" />
                    <StatCard label="Under review" value={stats.review} icon={<FileText className="w-4 h-4" />} color="blue" />
                    <StatCard label="Settled" value={stats.settled} icon={<CheckCircle className="w-4 h-4" />} color="green" />
                </div>
            )}
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                        ...
                        <p className="text-xs text-gray-400 mt-1 mb-4">
                            {search || filter !== 'ALL' ? 'Try adjusting your search or filter.' : 'File your first claim to get started.'}
                        </p>
                        {!search && filter === 'ALL' && (
                            <Link
                                href="/hospital/claims/new"
