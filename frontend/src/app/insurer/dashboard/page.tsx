...
export default function InsurerDashboard() {
    ...
    const stats = {
        queue: claims.filter((c) => c.status === 'SUBMITTED_TO_INSURER').length,
        review: claims.filter((c) => c.status === 'UNDER_REVIEW').length,
        approved: claims.filter((c) => c.status === 'APPROVED').length,
        rejected: claims.filter((c) => c.status === 'REJECTED').length,
        payout: claims.filter((c) => c.status === 'APPROVED').reduce((s, c) => s + (c.approvedAmount ?? 0), 0),
    }
    ...
    return (
        <>
            <PageHeader
                ...
                subtitle="Review incoming claims and manage your queue."
            />
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <StatCard label="In queue" value={stats.queue} icon={<Clock className="w-4 h-4" />} color="amber" />
                    ...
                    <StatCard label="Total payout" value={formatCurrency(stats.payout)} icon={<CheckCircle className="w-4 h-4" />} color="green" sub="approved claims" />
                </div>
            )}
